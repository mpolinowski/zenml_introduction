...
class LinearRegressionModel(Model):
    ...
    def train(self, X_train, y_train, **kwargs):
        ...
        try:
            ...
            # debugging
            logging.debug("DEBUG :: X_train shape: {}".format(type(X_train)))
            logging.debug("DEBUG :: y_train shape: {}".format(type(y_train)))

            reg_model.fit(X_train, y_train)
            logging.info("INFO :: Linear Regression model training completed")

            return reg_model

        except Exception as e:
            logging.error("ERROR :: Training the Linear Regression model failed: {}".format(e))
            raise e
